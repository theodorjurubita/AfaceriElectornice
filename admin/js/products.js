...
$(document).ready(function () {
    ...
    readCategories(); // read the categories
});
...
function readCategories(){
    $.get("/categories/", {}, function(data, status){
        let categoriiHtml = '<option value="">Select the category</option>';
        data.forEach(function(value){
            categoriiHtml += '<option value="' + value.id + '" name="' + value.name + '">'+ value.name + '</option>';
        })
        
        $("#category_id").html(categoriiHtml);
    })
}
...
function saveRecord() {
    ...
    formData.category_id = $("#category_id option:selected").val();
    
    var category = {
        name: $("#category_id option:selected").attr('name')
    };
